...
class Physics
{
	public:
		...
		void addBox(Ogre::SceneNode *ogreNode, btVector3 boxDimensions, btScalar mass, btVector3 inertia, btScalar damping)
		{
			...
			int *lf = new int;
			*lf = objectLifeTime;
			bodies.insert(std::pair<Ogre::Node *, BodyParameters>(ogreNode, { mass, inertia, motionState, rigidShape, body, ogreNode, lf }));
		}
};
...
int main()
{
	...
	Ogre::MaterialPtr bumpyMaterial = Ogre::MaterialManager::getSingleton().getByName("Examples/OgreLogo");
	...
	// sky
	sceneManager->setSkyBox(true, "Examples/TrippySkyBox", 99, false);
	...
	physics.dynamicsWorld->addRigidBody(groundBody);
